...
void Engine::incrementModel(){
  ...
  currentModel = static_cast<size_t>(currentModel + 1) % modelInstances.size();
}

void Engine::decrementModel(){
  if (modelInstances.empty()) return;
  unsigned int size = static_cast<unsigned int>(modelInstances.size());
  currentModel = (currentModel + size - 1) % size;
}
