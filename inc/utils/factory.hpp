...
bool createTriangle(VAOManager* vaoManager, const std::string& name, unsigned int shaderID, const Vec2& size = defaultSize2D, const Vec4& vertexColour = {1.0f, 1.0f, 1.0f, 1.0f});
bool createSquare(VAOManager* vaoManager, const std::string& name, unsigned int shaderID, const Vec2& size = defaultSize2D);
...
bool createMeshGridFromPath(Engine& engine, const std::string& baseName, const std::string& path, int startIndex, int count, const Vec2& spacing, const Vec3& rotation = defaultRotation, const Vec3& scale = defaultScale, bool hasNormals = false);
